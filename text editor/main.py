...
root=Tk()
root.title('Text Editor (Created by Neha)')
...
fontfamily_Combobox=Combobox(tool_bar,width=20,values=font_families,state='readonly',textvariable=font_family_variable)
fontfamily_Combobox.current(font_families.index('Arial'))
fontfamily_Combobox.grid(row=0,column=0,padx=5)
size_variable=IntVar()
font_size_Combobox=Combobox(tool_bar,width=10,textvariable=size_variable,state='readonly',values=tuple(range(8,81)))
...
textarea=Text(root,yscrollcommand=scrollbar.set,font=('arial',20),undo=True)
